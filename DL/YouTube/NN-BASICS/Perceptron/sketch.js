...
function setup() {
  ...
  ptron = new SimplePerceptron(3, 0.001); // Learning Constant is low just b/c it's fun to watch, this is not necessarily optimal
...
function draw() {
  ...
  ptron.trainThePerceptron(training[count].input, training[count].output);
  ...
  for (let i = 0; i < count; i++) {
    ...
    strokeWeight(1);
    fill(255);
    let guess = ptron.feedforwardIntoNN(training[i].input);
    if (guess > 0) noFill();
